...
for (x = 0; builtints[x].type; x++)
if (_strcmp(info->argv[0], builtints[x].type) == 0)
{
info->lines++;
builtin_return_value = builtints[x].func(info);
break;
}
return (builtin_return_value);
}
...
for (x = 0, word_count = 0; info->arg[x]; x++)
